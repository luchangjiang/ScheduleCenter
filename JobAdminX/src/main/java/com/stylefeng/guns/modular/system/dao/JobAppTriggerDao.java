...
public interface JobAppTriggerDao {
	...
	@Select("<script> " +
			...
			"order by job_create_time desc limit 300 " +
			"</script>")
	List<JobTriggerAppVo> getJobTriggerAppList(JobTriggerAppVo jobTriggerAppVo);

	@Select("SELECT d.id id, d.job_name job_name, d.job_code job_code, d.job_desc job_desc, d.job_create_time job_create_time, d.app_key app_key, d.job_type, d.result_wait_time, d.result_url, c.cron_expression cron_expression, d.callback_url, t.trigger_state  FROM QRTZ_JOB_EXTEND d, QRTZ_TRIGGERS t, QRTZ_CRON_TRIGGERS c WHERE d.job_name = t.job_name AND d.job_group = t.job_group AND t.trigger_name = c.trigger_name AND t.trigger_group = c.trigger_group")
	...
	@Select("<script> " +
			...
			"</if>" +
			"<if test = 'appKey != null and appKey.length > 0'>" +
	...
	@Update("UPDATE QRTZ_APP_INFO SET app_name = #{appName}, app_secret = #{appSecret}, app_status = #{appStatus} WHERE app_key = #{appKey}")
	Integer updAppByAppKey(AppVo vo);
	...
	@Update("CREATE TABLE IF NOT EXISTS ${tableName} (\n" +
			...
			"  `result_wait_time` INT(11) NOT NULL DEFAULT '0' COMMENT '结果等待时间',\n" +
			"  `result_update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '结果更新时间',\n" +
			"  PRIMARY KEY (`id`),\n" +
